...
module.exports = (app,auth,logger)=>{
	...
	*/
	...
	app.post("/API/DETAIL_BLOCK", auth.authCheck('all'), async (req,res)=>{
		...
		console.log("[SPREAD BLOCK]");
		await db.sequelize.query("select t1.*, t2.* from BLOCK_ISSUEs t1 join USERs t2 on t1.UID = t2.UID where t1.SHOW = 'SHOW' and PID = '"+req.body.PID+"'").spread((result)=>{ ParentBlocks= makeSpreadArray(result); });
		...
		await db.sequelize.query("select t1.*, t2.* from BLOCK_ISSUEs t1 join USERs t2 on t1.UID = t2.UID where t1.SHOW = 'SHOW' and PPID = '"+req.body.PID+"' order by t1.ID DESC").spread((result)=>{ ChildBlocks= makeSpreadArray(result); });
