$(document).ready(function () {
    ...
    $("#radioCustom1").on('change',function(){
        console.log($(this).val());
        $(".C_EXIST").css('display','none')
        $(".C_NONEXIST").css('display','block')
    })

    $("#radioCustom2").on('change',function(){
        console.log($(this).val());
        
        $(".C_EXIST").css('display','block')
        $(".C_NONEXIST").css('display','none')
    })
    ...
    $("#register").on('click',function(){
        console.log("등록!");
        var target_input_group = $(".form-group-material input");
        var target_select_group = $(".form-group-material select");
        
        let data = {};
        var exist = $(":input:radio[name=a]:checked").val();
        ...
        $.each(target_input_group,function(index,input){
            ...
            let label = $(input).attr('name');
            // if(value == ''){
            //     alert(label+" 필드 값을 입력해주세요!");
            //     throw "Throw error: No Data"
            // }
            data[label] = value
        })
        console.log(data)
        if(data['C_PASSWORD'] !== data['C_PASSWORD_CONFIRM']){
